...
def save_author_paper_venue(path='../feature/'):
    ...
    for name, author in name2author.items():
        ...
        if not author.coauthor_train_citation:
            ...
            author.coauthor_train = sum(
                author.coauthor_train_citation)/len(author.coauthor_train_citation)
    ...
    def set_topics():
        ...
        lda = LatentDirichletAllocation(
            n_topics=no_topics, max_iter=5, learning_method='online', learning_offset=50., random_state=0).fit_transform(tf)
...
def feature_venue_rank(author):
    author.venue_ranks = [name2venue[id2paper[paper].conference]
                          .rank for paper in author.papers if id2paper[paper].conference]
